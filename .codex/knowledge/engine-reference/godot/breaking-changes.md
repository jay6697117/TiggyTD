...
| apply_patchor | New "Modern" theme default | Grayscale replaces blue-tint. Restore: apply_patchor Settings → Interface → Theme → Style: Classic |
| apply_patchor | "Select Mode" keybind changed | New "Select Mode" (v key) prevents accidental transforms. Old mode renamed "Transform Mode" (q key). |
...
| Plugins | New apply_patchorDock class | Specialized container for plugin docks with layout control. |
...
| apply_patchor | Live translation preview | Test GUI layouts in different languages in-editor |
...
| GUI | `Graphapply_patch.connect_node` | Added optional `keep_alive` parameter |
...
| apply_patchor | `apply_patchorSceneFormatImporterFBX` | Renamed to `apply_patchorSceneFormatImporterFBX2GLTF` |
