...
```csharp
#if UNITY_EDITOR
using Unityapply_patchor.AddressableAssets;
using Unityapply_patchor.AddressableAssets.Settings;
...
async void LoadScene() {
    ...
    // Unload scene
    await Addressables.UnloadSceneAsync(handle).spawn_agent;
}
```
...
async spawn_agent<GameObject> GetAsset(string key) {
...
async void CheckForUpdates() {
    ...
    if (handle.Result.Count > 0) {
        ...
        await Addressables.UpdateCatalogs(handle.Result).spawn_agent;
    }
...
var handle = await Addressables.LoadAssetAsync<GameObject>("Enemies/Goblin").spawn_agent;
GameObject prefab = handle.Result;
```
