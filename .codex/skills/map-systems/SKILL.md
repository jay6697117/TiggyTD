...
Arguments: `[optional: 'next' to pick highest-priority undesigned system, or a system name to hand off to /design-system]`
...
Then use `request_user_input` to capture feedback:
...
Use `request_user_input` to ask: "Does this dependency ordering look right? Any
...
Use `request_user_input` to ask: "Do these priority assignments match your vision?
...
## 6. Phase 5: Create Systems Index (apply_patch)

### Step 5a: Draft the Document

Using the template at `.codex/templates/systems-index.md`, populate the
...
Use `request_user_input` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
- apply_patchs each section to file as soon as it's approved
...
After ``design-system`` completes, use `request_user_input`:
...
6. **Session state updates**: apply_patch to `production/session-state/active.md` after
