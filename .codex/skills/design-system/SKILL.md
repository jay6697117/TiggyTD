...
Arguments: `<system-name> (e.g., 'combat-system', 'inventory', 'dialogue')`
...
Use the template structure from `.codex/templates/game-design-document.md`:
...
- spawn_agent: Designing [system-name] GDD
...
```
Context  ->  Questions  ->  Options  ->  Decision  ->  Draft  ->  Approval  ->  apply_patch
```
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `request_user_input` for constrained questions, conversational text for open-ended
   exploration.
...
5. **Draft**: apply_patch the section content in conversation text for review. Flag any
   provisional assumptions about undesigned dependencies.

6. **Approval**: Ask "Approve this section, or would you like changes?"

7. **apply_patch**: Use the apply_patch tool to replace the `[To be designed]` placeholder with
...
**Agent delegation**: For complex mechanics, use the spawn_agent tool to delegate to
...
delegate to `systems-designer` via the spawn_agent tool. Provide:
...
- The main session presents the agent's output to the user via `request_user_input`
