...
Arguments: `[target-phase: systems-design | technical-setup | pre-production | production | polish | release]`
...
**Required Artifacts:**
- [ ] Engine chosen (.codex/docs/project-instructions.md Technology Stack is not `[CHOOSE]`)
- [ ] Technical preferences configured (`.codex/docs/reference/technical-preferences.md` populated)
- [ ] At least 1 Architecture Decision Record in `docs/architecture/`
- [ ] Engine reference docs exist in `.codex/knowledge/engine-reference/`
...
**Quality Checks:**
- [ ] Tests are passing (run test suite via shell)
...
### Quality Checks
- For test checks: Run the test suite via `shell` if a test runner is configured
...
1. apply_patch the new stage name to `production/stage.txt` (single line, no trailing newline)
...
5. **Get approval**: "May I write this gate check report to production`gate-check`s/?"
