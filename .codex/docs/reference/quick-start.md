...
| apply_patch combat code | `gameplay-programmer` |
| Create a shader | `technical-artist` |
| apply_patch dialogue | `writer` |
| Plan the next sprint | `producer` |
| Review code quality | `lead-programmer` |
| apply_patch test cases | `qa-tester` |
...
| apply_patch Unity shaders/VFX | `unity-shader-specialist` |
| Manage Addressable assets | `unity-addressables-specialist` |
| Build UI Toolkit/UGUI screens | `unity-ui-specialist` |
| apply_patch idiomatic GDScript | `godot-gdscript-specialist` |
...
| apply_patch patch notes for players | `community-manager` |
| Brainstorm a new game idea | Use ``brainstorm`` skill |

### 3. Use Slash Commands for Common spawn_agents
...
2. **Set up the engine** — Run ``setup-engine`` (uses the brainstorm recommendation)
   - Configures .codex/docs/project-instructions.md, detects knowledge gaps, populates reference docs
   - Creates `.codex/docs/reference/technical-preferences.md` with naming conventions,
...
2. **apply_patch the Game Pillars** — delegate to `creative-director`
...
```
.codex/docs/project-instructions.md                          -- Master config (read this first, ~60 lines)
.codex/
  ...
  docs/
    ...
    setup-requirements.md          -- System prerequisites (Git shell, jq, Python)
