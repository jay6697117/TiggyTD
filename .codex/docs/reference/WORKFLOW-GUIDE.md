...
- **Git** with Git shell (Windows) or standard terminal (Mac/Linux)
...
- Pins the engine and version in `.codex/docs/project-instructions.md`
- Populates `.codex/docs/reference/technical-preferences.md` with naming conventions,
...
- Creates version-pinned reference docs in `.codex/knowledge/engine-reference/`
...
> `.codex/docs/project-instructions.md` directly if you prefer.
...
If you see this, hooks are working. If not, check `.codex/config.toml` to
make sure the hook paths are correct for your OS.

### Step 0.4: Create Your Directory Structure

The directories listed in `.codex/docs/project-instructions.md` don't all exist yet. Create them as
...
.codex/templates/game-concept.md
```
...
.codex/templates/game-pillars.md
```
...
.codex/templates/game-design-document.md
```

**The `design-docs` rule** (`.codex/docs/reference/rules/design-docs.md`) requires every GDD
...
The `narrative` rule (`.codex/docs/reference/rules/narrative.md`) enforces:
...
.codex/templates/milestone-definition.md
```
...
**Key rule:** The `prototype-code` rule (`.codex/docs/reference/rules`prototype`-code.md`)
...
5. apply_patch tests (verification-driven development)
...
- spawn_agents added since sprint start
...
### Step 6.1: apply_patch Tests First (Verification-Driven Development)

The project enforces **verification-driven development**: write tests before
or alongside implementation, not after.

The `test-standards` rule (`.codex/docs/reference/rules/test-standards.md`) enforces:
...
Agent: [apply_patchs tests following test-standards rule: proper naming, Arrange/Act/Assert
...
.codex/templates/post-mortem.md
```
...
| apply_patch test cases | `qa-tester` | 3 |
...
6. apply_patch tests alongside code
