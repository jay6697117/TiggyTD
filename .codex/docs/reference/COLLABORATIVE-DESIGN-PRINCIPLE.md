...
## 📋 How This Applies to Different spawn_agents
...
Agent: [apply_patchs design/gdd/combat-system.md with full design]
...
Agent: [apply_patchs src/gameplay/combat/damage_calculator.gd]
Agent: "Implemented!"
...
Agent: [apply_patchs code, runs through gameplay-code rule checks, fixes issues]
...
1. **Explain first** — apply_patch your full expert analysis in conversation text:
...
- When running as a spawn_agent subagent (tool may not be available)
...
### NEVER apply_patch Files Without Explicit Approval
...
3. IF User says "Yes":
   Agent: [Uses apply_patch/apply_patch tool]
...
2. For EACH section:
   ...
   User: "Yes"
   Agent: [apply_patchs section into file]
...
Agent: "This implementation requires changes to 3 files:
       ...
       C) apply_patch all 3 now (fastest, but less review)
...
- **.codex/docs/project-instructions.md** — Collaboration protocol section added
