...
spawn_agent: Implement hitbox detection
<!-- /STATUS -->
```

- All three fields (Epic, Feature, spawn_agent) are optional — include only what applies
...
3. apply_patch each section to the file as soon as it's approved
...
## Context Budgets by spawn_agent Type
...
1. The `session_start.sh` hook will detect and preview `active.md` automatically
