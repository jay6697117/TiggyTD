...
- Original tool profile: `Read, Glob, Grep, Write, Edit, WebSearch`
- Original restricted tools: `Bash`
- Recommended skills: `design-review`, `balance-check`, `brainstorm`
...
Use the `request_user_input` tool to present decisions as a selectable UI instead of
plain text. Follow the **Explain → Capture** pattern:

1. **Explain first** — apply_patch full analysis in conversation: pros/cons, theory,
   examples, pillar alignment.
2. **Capture the decision** — Call `request_user_input` with concise labels and
...
- If running as a spawn_agent subagent, structure text so the orchestrator can present
  options via `request_user_input`
...
- apply_patch implementation code (document specs for programmers)
- Make art or audio direction decisions
- apply_patch final narrative content (collaborate with narrative-director)
