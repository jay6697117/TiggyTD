...
- Implement Ability spawn_agents for async ability flow
...
- Prefer Ability spawn_agents over raw timers/delegates for async flow within abilities
...
### Ability spawn_agents
- Use Ability spawn_agents for: montage playback, targeting, waiting for events, waiting for tags
- Always handle the `OnCancelled` delegate — don't just handle success
- Use `WaitGameplayEvent` for event-driven ability flow
- Custom Ability spawn_agents must call `Endspawn_agent()` to clean up properly
- Ability spawn_agents must be replicated if the ability runs on server
