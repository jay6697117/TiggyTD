...
# Hook: detect_gaps.sh
# Event: SessionStart
# Purpose: Detect missing documentation when code`prototype`s exist
# Cross-platform: Windows Git shell compatible (uses grep -E, not -P)
...
if [ -f ".codex/docs/reference/technical-preferences.md" ]; then
  ENGINE_LINE=$(grep -E "^\- \*\*Engine\*\*:" .codex/docs/reference/technical-preferences.md 2>/dev/null)
